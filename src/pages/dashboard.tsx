...
import { useFetchProductQuery, useRemoveProductMutation } from "../services/product.services"
...
const Dashboard = () => {
    ...
    return <div className="overflow-x-auto container mx-auto">
        ...
        <Link to="/add" className="p-2 bg-green-600">Thêm mới</Link>
        <table className="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
            ...
            <tbody className="divide-y divide-gray-200 ">
                {data?.map((item, index) => <tr key={item.id}>
                    ...
                    <td>
                        <button className="bg-yellow-500 p-2"><a href={`/update/${item.id}`} >Update</a></button>
                        <button onClick={() => handleDelete(item.id)} className="bg-red-500 p-2">Delete</button>
                    </td>
                </tr>)}
